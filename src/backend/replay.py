...
async def get_replay_async(rid: str) -> Optional[Dict[str, Any]]:
    replay_data = _REPLAY_CACHE.get(rid)
...
async def delete_replay_async(rid: str) -> None:
    _REPLAY_CACHE.pop(rid, None)
    try:
        from .db import db
...
def get_replay_summary(rid: str) -> Optional[Dict[str, Any]]:
    ...
    replay_events: List[Dict[str, Any]] = replay_data.get("events", [])
    event_counts = Counter(event.get("type", "unknown") for event in replay_events)
    return {
